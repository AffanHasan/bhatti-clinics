...
class DailyOPD extends PolymerElement {
  ...
  List consultants = [
                       { "_id" : 1, "name" : "Dr. Imtiaz Bhatti", "daily_opd_rate%" : 60 },
                       { "_id" : 2, "name" : "Dr. Khalid Bhatti", "daily_opd_rate%" : 60 },
                       { "_id" : 3, "name" : "Dr. Mumtaz Laghari", "daily_opd_rate%" : 70 }
                     ];
...
//Event Handler Function Definitions Starts
  void addEditLogEntryEH(Event e, var detail, Node target){
    ...
    switch(detail['action']){
      case "add":
        ...
        int consultantRate = consultants.firstWhere((item){ return item["_id"] == consultantId; })['daily_opd_rate%'];
  ...
  void addLogEntryFormInputsEH(Event e, var detail, Node target){
    ...
    var flag = false;
    viewData['log_entry_form'].forEach(
        (key, value){
  ...
  void editLogEntryEH(Event e, var detail, Node target){
    e.preventDefault();
    viewData['log_entry_form']['input_fields'].forEach(
        ( key, value ){
          switch(key){
            case "patient_info":
